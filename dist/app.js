...
(() => {
  ...
  const badge = e => `<span class="badge ${e.catalog ? 'reference' : e.type}">${e.catalog ? 'Source claim · no recipe' : e.type === 'guide' ? 'Preparation guide' : 'Reference only'}</span>`;
  ...
  function card(e) {
    return `<article class="card"><div class="card-art"><img src="${artwork[e.category]}" alt="${escape(artAlt(e))}" loading="lazy"><span class="art-caption">${e.type === 'guide' ? 'recipe method' : e.catalog ? 'source index' : 'source note'}</span></div><div class="card-top"><span class="category-label">${escape(e.concern)}</span><button class="save-button" data-save="${e.id}" aria-pressed="${state.saved.has(e.id)}" aria-label="${escape(savedLabel(e))}">${state.saved.has(e.id) ? '♥' : '♡'}</button></div><p class="condition-label">${e.catalog ? 'Source index indication' : 'Traditional indication'}: ${escape(e.condition)}</p><h3>${escape(e.condition)}</h3><p class="subtitle">${escape(e.name)} · ${escape(e.form)}</p><p class="botanical-name">${escape(e.botanical)}</p><p class="use">${escape(e.use)}</p>${badge(e)}<div class="card-bottom"><span>${escape(e.sourceLabel || 'WHO · chapter')} p. ${e.page}</span><button data-open="${e.id}" aria-label="Read ${escape(e.condition)} — ${escape(e.name)}">Read ${e.type === 'guide' ? 'recipe' : e.catalog ? 'source note' : 'reference'} ↗</button></div></article>`;
  }
  function render() {
    syncAilmentControls();
